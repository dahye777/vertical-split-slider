...
//splitTxt함수의 인수로 반복을 돌고있는 panel li의 안에 h2태그를 찾아서
//인수로 넣습니다
...
function moveUp() {
  ...
  //찾은 그 대상의 index를 구합니다 (indexOf) => panel_li_arr을 사용해야함
  ...
  setTimeout(() => {
    ...
    //동시에 이전에활성화 되었던  current_item에 붙인 up클래스를 지웁니다
  }, delay);
}
...
function splitTxt(el) {
  ...
  /*    textContent vs innerText
    ...
    가져오지 못합니다
    예) h2태그가있는데 span가 안에있습니다. span를 display : none;을 했을때
    innerText 는 span을 읽지 못합니다
    하지만 textContent는 가져옵니다    */

  const fragment = document.createDocumentFragment();
  /*
    DocumentFragment : DOM(document등으로 생성되는 요소들)조작을 메모리상에서
    수행하는 매서드,  메모리에서만 DOM을 구성하고 실제로 DOM에 삽입하는건 
  ...
  /*
    ...
    따라서 innerHTML을 사용하는 방법은 확실한 보안상태의 코드일때 사용하는것이
    좋습니다
    */
}
